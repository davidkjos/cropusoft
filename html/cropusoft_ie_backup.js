...
function readXmlFile(fileName){
	...
	$.ajax({
		...
		success: function(data, status){
			var countyVertices = $(data).find("county-limits");
			var countyPts = createPolygon(countyVertices,"county");
			$.ajax({
				url: '/',
				type: 'POST',
				data: {
	...
	//xmlDoc.open('GET', "/html/"+fileName);
	
	//return xmlDoc;
	}
	
	
function readXmlFile3(fileName){
	...
	$.ajax({
		...
		//},
		success: function(data, status){
			var countyLimitsXml = $(data).find("township-limits");
			var length = $(countyLimitsXml).length;
			for (var i=0; i<length; i++){
				var polygon = createPolygon($(data).find("township-limits:nth("+i+")"));
				var name = $(data).find("township:nth("+i+")").attr("name");
				$.ajax({
					url: '/',
					type: 'POST',
					data: {
...
function readXmlFile2(fileName){
			...
			for (var i=0; i<length; i++){
				...
				for (var j = 1; j < 37; j++) {
					...
					if (j<7 )
						...
						sectionNumber = 19-val;
					}
					else if (j>18 && j<25){
						sectionNumber = j;
					}
					else if (j>24 && j<31){
						sectionNumber = 31-val;
					}
					else if (j>30 && j<37){
						sectionNumber = j;
					}
					
					if (val==6 || val==18 || val==30){
						val=1;
					}
					else {
						val++;
					...
					var k=j-1;
					var sectionXML = $(data).find("township:nth("+i+")").find("section:nth("+k+")");
...
function createPolygon(vertices, type){
	...
	for (var i=0; i<length; i++){
		str = str +
			 "<vertex lat='"
			+ $(vertices).find("vertex:nth("+i+")").find("lat").text()
			+ "' lng='"
			+ $(vertices).find("vertex:nth("+i+")").find("long").text() 
			+ "' />";
	}
	str = str + "</vertices>"
	return str;
...
$.ajaxSetup({ cache: false });
	...
	$.ajax({
		...
		success: function(data){
			...
			try {
				...
				//var poly = createPolygon2(county);
				//map.addOverlay(poly);
				try {
					var length = $(doc).find("township").length;
					for (var i = 0; i < length; i++) {
						var poly = createPolygon2($(doc).find("township:nth(" + i + ")"));
						map.addOverlay(poly);
					}
				} 
				catch (e) {
				...
				var poly = createPolygon2($(doc).find("section[number=33]"));
				map.addOverlay(poly);
	...
	var farmsXml = null;//readXmlFile("farms2.xml");
	var fieldsXml = null;//$(farmsXml).find("site:first").find("field");
	var length = null;//$(fieldsXml).length;
